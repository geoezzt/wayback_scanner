  ...
  return new Promise(function(resolve, reject) {
    try {
      ...
      var req = lib.get(urlStr, {
        ...
        var data = '';
        res.on('data', function(c) { if (data.length < 1000000) data += c; });
...
module.exports = async function(req, res) {
  ...
  var limit = Math.min(parseInt(body.limit) || 1000, 2000);
  var scanSecrets = body.scan_secrets !== false;

  if (!domain) { res.status(400).json({ error: 'domain required' }); return; }
  var cleanDomain = domain.replace(/^https?:\/\//,'').replace(/\/.*/,'').replace(/^www\./,'').trim();

  // 1. Fetch from Wayback — single fast query
  var urls = {};
  try {
    var r = await fetchRaw(
      'https://web.archive.org/cdx/search/cdx?url=' + cleanDomain + '/*&output=text&fl=original&collapse=urlkey&limit=' + limit,
      20000
    );
    r.body.split('\n').forEach(function(u) {
      var t = u.trim();
  ...
  try {
    var r2 = await fetchRaw(
      'https://web.archive.org/cdx/search/cdx?url=*.' + cleanDomain + '/*&output=text&fl=original&collapse=urlkey&limit=500',
      10000
    );
    r2.body.split('\n').forEach(function(u) {
      var t = u.trim();
  ...
  var urlList = Object.keys(urls).slice(0, limit);

  // 2. Classify all
  var classified = urlList.map(function(url) {
    return { url: url, tags: classifyUrl(url), status: null };
  });

  // 3. Status check — parallel batch of 100 max
  var toCheck = classified.slice(0, 100);
  var statusRes = await Promise.allSettled(toCheck.map(function(r) { return headCheck(r.url); }));
  statusRes.forEach(function(r, i) {
    if (r.status === 'fulfilled') toCheck[i].status = r.value.status;
  });

  // 4. Secret scan — max 20 JS/JSON files
  var allSecrets = [];
  if (scanSecrets) {
    var toScan = classified
      .filter(function(r) { return r.tags.includes('js') || r.tags.includes('json') || r.tags.includes('env'); })
      .slice(0, 20);

    // scan main page
    try {
      var main = await fetchRaw('https://' + cleanDomain, 6000);
      allSecrets = allSecrets.concat(scanForSecrets(main.body, 'https://' + cleanDomain));
    } catch(e) {}

    // scan JS files in parallel
    await Promise.allSettled(toScan.map(async function(item) {
      try {
        var res2 = await fetchRaw(item.url, 6000);
        var secrets = scanForSecrets(res2.body, item.url);
        if (secrets.length) allSecrets = allSecrets.concat(secrets);
        item.status = res2.status;
      } catch(e) {}
    }));
  }
  ...
  var buckets = {
    ...
    server_error: classified.filter(function(r) { return r.status >= 500; }),
  ...
  var special = {
    ...
    sensitive_files:    classified.filter(function(r) {
      return r.tags.some(function(t) { return ['env','git','database','log','backup','config','archive'].includes(t); });
    }),
